...
export class MatchPreferenceService {
  constructor(@InjectModel(MatchPreference.name) private matchPreferenceModel: Model<MatchPreferenceDocument>) {
  }

  async createMatchPreference(
    createMatchPreferenceDto: CreateMatchPreferenceDto
  ): Promise<MatchPreference> {
    const createdMatchPreference = new this.matchPreferenceModel(createMatchPreferenceDto);
  ...
  async updateMatchPreference(id: string, updateMatchPreferenceDto: UpdateMatchPreferenceDto): Promise<MatchPreference> {
    return this.matchPreferenceModel.findByIdAndUpdate(id, updateMatchPreferenceDto, { new: true }).exec();
  }

  async deleteMatchPreference(id: string): Promise<boolean> {
    const result = await this.matchPreferenceModel.deleteOne({ _id: id }).exec();
